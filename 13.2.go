...
func DaftarkanBuku(pustaka *DaftarBuku, n int) {
	...
	for i := 0; i < n; i++ {
		...
		fmt.Print("Masukkan Judul Buku (gunakan underscore untuk spasi): ")
		fmt.Scan(&buku.judul)

		fmt.Print("Masukkan Nama Penulis (gunakan underscore untuk spasi): ")
		fmt.Scan(&buku.penulis)

		fmt.Print("Masukkan Nama Penerbit (gunakan underscore untuk spasi): ")
...
func CetakTerfavorit(pustaka DaftarBuku, n int) {
	...
	buku := pustaka[maxIdx]
	fmt.Printf("Buku terfavorit adalah: %s %s %s %d\n",
...
func Cetak5Terbaru(pustaka DaftarBuku, n int) {
	...
	if n < 5 {
		...
		fmt.Println("Menampilkan 5 buku dengan rating tertinggi:")
	}
